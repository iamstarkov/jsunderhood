...
const moveMinorsToOther = pipe(groupBy( i => length(i) < 5 ? 'other' : parse(head(i)).host ), mapObjIndexed(flatten));
const hostAndLinksObj = pipe(toPairs, map(zipObj(['host', 'links'])));
const moveOtherToEnd = sortBy(group => group.host === 'other');
const getLinks = pipe(extractLinks, filterTwitterLinks, groupByHost, moveMinorsToOther, hostAndLinksObj, moveOtherToEnd);
